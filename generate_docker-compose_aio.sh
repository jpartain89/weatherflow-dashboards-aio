...
services:
  ...
  wxfdashboardsaio_influxdb:
    ...
    environment:
      TZ: ${timezone[$station_number]}
      INFLUXDB_ADMIN_PASSWORD: $(op item get WeatherFlowDashboards --field label=INFLUXDB_ADMIN_PASSWORD)
      ...
      INFLUXDB_USER_PASSWORD: $(op item get WeatherFlowDashboards --field label=INFLUXDB_ADMIN_PASSWORD)
    image: influxdb:1.8
    ports:
    - protocol: tcp
      published: 8086
